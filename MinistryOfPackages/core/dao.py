...
class PyPIData(object):
    ...
    def populate_initial_valid_classifiers(self):
        ...
        I intend to actually call that url in the setup, so while pypi *does*
        need to be available to do the initial population, it won't be necessary
        in a running service. The alternative is to check the url on an ongoing
        basis, which is clearly worse :)
        """
        pass
